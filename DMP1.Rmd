...
# Background {-}
The field area of this investigation is illustrated below.

![Figure 1](img/rio-grande-basin.png){width=50%}


[Map Source: @Bartolino_1979]

```{r table1, echo=TRUE}
usgs <- read.csv("data/usgs.csv") # This is a comment. 
head(usgs, 1)
```
This dataset has `r nrow(usgs)` sampling sites. 

```{r plot1, echo=TRUE}
plot(faithful, main= "Do I wait longer for eruptions?")
```
...
will be created that defines the relational structure of the database^[A metadata record will be produced using the FGDC standard that describes
...
with University/Participating institutional and NSF policies [See, e.g., @Briney_2015]. Depending
